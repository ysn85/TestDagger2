...
public class RulerView extends View implements ScrollChange {
     ...
     * 画刻度的Paint
     */
    private Paint mBiggerLinePaint;
    ...
    private void init(Context context) {
        ...
        mTextPaint.setColor(Color.parseColor("#5E6266"));
        mTextPaint.setTextAlign(Paint.Align.CENTER);

        final int lineColor = Color.parseColor("#BCC4CC");
        mLinePaint = new Paint();
        mBiggerLinePaint = new Paint();
        mLinePaint.setAntiAlias(true);
        mLinePaint.setStrokeWidth(mLineWidth);
        mLinePaint.setStyle(Paint.Style.FILL);
        mLinePaint.setColor(lineColor);

        mBiggerLinePaint.setAntiAlias(true);
        mBiggerLinePaint.setStrokeWidth(mBiggerLineWidth);
        mBiggerLinePaint.setStyle(Paint.Style.FILL);
        mBiggerLinePaint.setColor(lineColor);
    ...
    private void initDistanceForDp() {
        mLineWidth = dp2px(1);
        mBiggerLineWidth = mLineWidth * 2;
        mLineSpace = dp2px(4.5F);
        mSmallLineHeight = dp2px(13);
        mLongLineHeight = dp2px(32);
        dpFor05 = dp2px(0.5f);
        dpFor14 = dp2px(48);
    }

    public int getBiggerLineWidth() {
        return mBiggerLineWidth;
    }
    ...
    public void setCurrentItem(int index) {
        mRulerHelper.setCurrentItem(index);
    }
    ...
    private void drawRuler(Canvas canvas) {
        mTextIndex = 0;
        for (int index = 0, count = mRulerHelper.getCounts(); index <= count; index++) {
            ...
            Paint paint;
            if (longLine) {
                paint = mBiggerLinePaint;
                mRect.right = mRect.left + mBiggerLineWidth;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (Math.abs(xVelocity) >= mMinVelocity) {
                    ...
                    Log.i("TEST", "finalX = " + finalX + " centerPointX = " + centerPointX + " mCountRange = " + mCountRange);
